...
use crate::{check::{state::CheckState, SemanticToken, TokenKind}, item::AstItem, parser::expr::field::Field};
...
impl AstItem for Field {
    ...
    where
        ...
        D::Doc: Clone,
        A: Clone {
        self.struct_.0.pretty(allocator).append(".").append(&self.name.0)
    }
    ...
    fn hover<'db>(
        ...
        Some(format!("{}: {}", self.name.0, ty.get_name_with_types(state, type_vars)))
    }
