use crate::{parser::common::type_::Type, ty::Ty};
...
impl Type {
    pub fn type_resolve(&self, state: &mut TypeResolveState) -> Ty {
        match &self {
            Type::Named(named) => named.type_resolve(state),
            Type::Tuple(tup) => {
                let mut tys = vec![];
                for (ty, _) in tup {
                    tys.push(ty.type_resolve(state));
                }
                Ty::Tuple(tys)
            }
            Type::Sum(tup) => {
            ...
            Type::Function {
                ...
                receiver: receiver
                    .as_ref()
                    .map(|receiver| Box::new(receiver.as_ref().0.type_resolve(state))),
                args: args.iter().map(|r| r.0.type_resolve(state)).collect(),
                ret: Box::new(ret.0.type_resolve(state)),
            },
            Type::Wildcard(_) => {
                let id = state.new_type_var();
                Ty::TypeVar { id }
            }
        }
    }
